...
import Image, ImageColor, os.path, types, pdb # pyfits
...
"""
    ...
    if not overwrite and os.path.exists(output_file):
        ...
        f.write(imagefunc(transpose(fits.open(file)[hdu].data, axes=(1,2,0)), **kwargs))
        f.close()
...
def merge_all(files, suffixes, output_file, overwrite=False, \
    ...
    inf=[fits.open(f) for f in files]
    if screen==None:
    ...
    for i in range(ncam):
      ...
      # file is truncated
      if screenfile != None:
        attn = calculate_screen(fits.open(screenfile), i, **kwargs)
